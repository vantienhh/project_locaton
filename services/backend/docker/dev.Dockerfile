...
RUN apt-get update \
    && apt-get install -y libfreetype6-dev libjpeg62-turbo-dev zlib1g-dev libpng-dev libpq-dev libzip-dev libicu-dev libgmp-dev libsodium-dev libmcrypt-dev zip curl \
    && pecl install -o -f redis mcrypt-1.0.2 \
    && rm -rf /tmp/pear \
...
#    && docker-php-ext-configure gd --with-freetype-dir=/usr/include/ --with-jpeg-dir=/usr/include/ \
    ...
    && docker-php-source extract \
    && docker-php-ext-install pdo pdo_mysql zip sockets gmp sodium \
    && docker-php-source delete \
...
RUN apt-get update && apt-get upgrade -y && apt-get autoremove -y \
    && apt-get install -y git curl nano htop \
    && pecl install xdebug \
    && rm -rf /tmp/pear \
    && echo "zend_extension=$(find /usr/local/lib/php/extensions/ -name xdebug.so)\n" >> $PHP_INI_DIR/conf.d/xdebug.ini \
    && echo '[Xdebug]\n' >> $PHP_INI_DIR/php.ini \
    && echo 'xdebug.remote_host=host.docker.internal\n' >> $PHP_INI_DIR/php.ini \
    && echo 'xdebug.remote_port=9123\n' >> $PHP_INI_DIR/php.ini \
